mvr_dcv <-
function(formula,ncomp,data,subset,na.action,
  method = c("kernelpls", "simpls", "oscorespls", "svdpc"),scale=FALSE,repl=100,sdfact=2,
...
require(pls)
    ...
    method <- match.arg(method, c("kernelpls", "simpls", "oscorespls", "svdpc", "model.frame"))
    ...
    nobj <- dim(X)[1]
    npred <- dim(X)[2]
    if (length(attr(mt, "term.labels")) == 1 && !is.null(colnames(mf[[attr(mt, "term.labels")]])))
    ...
    ncomp <- min(ncomp, dx[1] - max(sapply(segments0, length)) - 1)
...
print(i)
       ...
       if (missing(length.seg0)) {
            ...
            segment0 <- cvsegments(dx[1], length.seg = length.seg0, type = segment0.type)
        }
      if (trace)
      ...
      for (n.seg0 in 1:length(segment0)) {
        ...
        #obsuse <- unlist(segment0)[unlist(segment0)!=seg0]
        res <- mvr(Y~X,ncomp=ncomp,subset=obsuse,na.action=na.action,method=method,
               scale=scale,segments=segments,segment.type=segment.type,trace=trace,
               validation="CV")

        # compute optimal number of components
        MSEPj <- matrix(NA,nrow=segments,ncol=ncomp)
        for (j in 1:segments){
          # predicted values for j-th part:
          predj <- res$vali$pred[res$vali$seg[[j]],,]
          # observation numbers in j-th part:
          obsj <- obsuse[res$vali$seg[[j]]]
          # residuals in j-th part:
          resj <- predj-Y[obsj]
          # MSEP in j-th part:
...
#        fvec <- (MSEPm>(min(MSEPm)+MSEPsd*sdfact))
if (selstrat=="diffnext"){
# compare MSE+factor*sd among neighbors, and if the MSE
# falls outside this bound, this is the optimal number
        fvec <- (diff(MSEPm)+sdfact*MSEPsd[-1])<0
        fvec <- c(TRUE,fvec) # original length of vector
        ind <- which.min(MSEPm)
        optcomp[n.seg0,i] <- max((1:ind)[fvec[1:ind]])
}
else if (selstrat=="hastie"){
# Hastie et al strategy:
# Search for the number of components with the minimum of the 
# mean MSE's. The optimal number of components is the model with
# the smallest number of components which is still in the range
# of the MSE+factor*sd, where MSE and sd are taken from the 
# minimum.
	ind <- which.min(MSEPm)
...
# Strategy with relative change combined with Hastie et al:
# First the minimum of the mean MSE's is searched, and MSE's of larger
# components are omitted. For this selection, the relative change in MSE
# compared to the min, and relative to the max, is computed. If this change
...
# Then the Hastie strategy is applied for the remaining MSE's.
	...
	plot(1:ncomp,MSEPm,xlab="Component number",ylab="Average MSEP",
             ylim=range(MSEPm, MSEPm + MSEPsd, MSEPm - MSEPsd),type="b")
             error.bars(1:ncomp, MSEPm + MSEPsd,MSEPm - MSEPsd, width = 1/ncomp,col=3)
	abline(v=optcomp[n.seg0,i],col=2)
	}

        predopt[seg0,,i] <- predict(res,data,ncomp=optcomp[n.seg0,i])[seg0,,]
    ...
    biasopt <- mean(resopt)
    SEPopt <- sqrt(sum((resopt-biasopt)^2)/(prod(dim(resopt))-1))
    ...
    list(resopt=resopt, predopt=predopt, optcomp=optcomp, pred = pred, SEPopt=SEPopt, 
         sIQRopt=sIQRopt, sMADopt=sMADopt, MSEPopt = MSEPopt)
}
