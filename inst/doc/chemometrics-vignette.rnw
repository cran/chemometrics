...
\setlength{\parskip}{0.4cm}       % Abstand zwischen Absätzen
...
shows the output of \code{pcaDiagplot} with the classic NIPALS PCA. Note that \verb+X.pca+ needs to 
be a \code{princomp} object or a list containing at least scores, loadings and the standard 
...
<<echo=FALSE, results=verbatim>>=
    cat(" SEP mean:               ", round(res.stepcv$SEPm, 4), "\n",
      "SEP median:             ", round(median(res.stepcv$SEP), 4), "\n",
      "SEP standard deviation: ", round(sd(res.stepcv$SEP), 4), "\n", sep=" ")
@
...
%<<echo=FALSE>>=
%  o <- apply(abs(res.stepcv$resid), 2,order)
%  trimlength <- floor(length(y)*0.8)
%  resid.trim.stepwise <- array(dim=c(trimlength,100))
%  for (i in 1:100) {
%    resid.trim.stepwise[,i] <- abs(res.stepcv$resid[,i])[o[1:trimlength,i]]
%  }
%@
...
%  for (i in 1:100) {
%    resid.trim.pcr[,i] <- abs(res.pcr$resopt[,1,i])[o[1:trimlength,i]]
%  }
%@
%-------------------------------------------------------------- pcr end.
...
(Figure \ref{fig:comppls}) is 9, %Sexpr{res.pls$afinal}
which is very low compared to the former 
results. We can also observe that the values for the number of used PLS components which occur 
...
%  for (i in 1:100) {
%    resid.trim.pls[,i] <- abs(res.pls$resopt[,1,i])[o[1:trimlength,i]]
%  }
@
%-------------------------------------------------------------- pls end.
...
The optimal number of components can be determined by repeated double CV in function \code{prm\_dcv}.

<<eval=FALSE>>=
  prm_dcv(X, y, ncomp=40)
@

Since RDCV is computationally rather expensive in this case, the function \code{prm\_cv} which 
accomplishes a single CV might be more convenient for our task:
...
In our case it is the model with 17 %Sexpr{res.prmcv$optcomp} 
components have a 20\% trimmed SEP of 3.77. %Sexpr{round(res.prmcv$SEPop, 2)}

<<echo=FALSE, results=verbatim>>=
  oc <- 17
  cat(" optimal number of PCs: 17 \n",  #oc <- res.prmcv$optcomp
       "                         classic   20% trimmed \n",
       "SEP mean:                5.382      3.9253 \n",          #round(mean(res.prmcv$SEPj[,oc]), 4), round(mean(res.prmcv$SEPtrimj[,oc]), 4)
       "SEP median:              5.2846     3.8311 \n",          # round(median(res.prmcv$SEPj[,oc]), 4), round(median(res.prmcv$SEPtrimj[,oc]), 4)
       "SEP standard deviation:  0.9359     0.6934 \n", sep=" ") #round(sd(res.prmcv$SEPj[,oc]), 4), round(sd(res.prmcv$SEPtrimj[,oc]), 4)
@
...
it becomes clear that there are no outliers in our data. In absence of outliers, the robust method 
is never better than the non-robust version.
...
\begin{equation*}
  \ef \text{ \midtilde } N(\bf{0}, \sigma^2 \Id).
\end{equation*}
...
Ridge regression leads to an average SEP of 5.39 %Sexpr{round(res.ridge$SEPm, 2)} 
which is already a
...
%  for (i in 1:100) {
%    resid.trim.ridge[,i] <- abs(res.ridge$residuals[,i])[o[1:trimlength,i]]
%  }
%@
...
(BLUE), there cannot be another method giving better results indeed.

\newpage
                                                         
<<echo=FALSE, results=verbatim>>=
  SEP.stepwise <- 4.625062 #res.stepcv$SEPm
  SEP20.stepwise <- 1.508322 #mean(apply(resid.trim.stepwise, 2, sd))
  SEP.pcr <- 7.505016 #mean(apply(res.pcr$resopt[,1,], 2, sd))
  SEP20.pcr <- 2.587331 #mean(apply(resid.trim.pcr, 2, sd))
  SEP.pls <- 6.399661 #mean(apply(res.pls$resopt[,1,], 2, sd))
  SEP20.pls <- 2.241617 #mean(apply(resid.trim.pls, 2, sd))
  SEP.prm <- 5.708654 #res.prmcv$SEPall[res.prmcv$optcomp]
  SEP20.prm <- 3.768013 #res.prmcv$SEPop
  SEP.ridge <- 5.39256 #res.ridge$SEPm
  SEP20.ridge <- 1.833630 #mean(apply(resid.trim.ridge, 2, sd))
  SEP.lasso <- 5.660052 #res.lasso$SEP[(res.lasso$fraction==res.lasso$sopt)]
  SEP20.lasso <- 1.950366 #sd(y - as.matrix(X)%*%res.lassocoef$coef)
  cat(" PREDICTION PERFORMANCE",
    ...
    "\n PRM         ", round(SEP.prm, 2), "    ", round(SEP20.prm, 2), "        ", oc, " components",
...
<<echo=FALSE, results=verbatim>>=
  cat(" COMPUTATION TIMES \n",
    ...
    "PCR         100 x RDCV    8min 14sec \n",
    "PLS         100 x RDCV    5min 20sec \n",
    "PRM         single CV     1min 15sec \n",
    "Ridge       100 x RCV     2min 36sec \n",
    "Lasso       single CV     1min 03sec \n")
@
